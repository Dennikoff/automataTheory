...
class NodeBuilder:
    ...
    def statement_list(self, p):
        if len(p) == 3:
            ...
            p[0] = Node('new line', data=[p[1]], lineno=p.lineno(1))

    def single_statement(self, p):
        if len(p) != 3:
            p[0] = p[1]
        else:
            p[0] = Node('End', lineno=p.lineno(1))
    ...
    def math_expr(self, p):
        if len(p) == 4:
            ...
            if p[2] == '|':
                p[0] = Node('compare', data=p[4], children=[p[1], p[3]], lineno=p.lineno(1))
            else:
    ...
    def if_b(self, p):
        if p[7] == ';':
            p[0] = Node('if then', children=[p[2], p[5]], lineno=p.lineno(1))
        else:
            p[0] = Node('if then else', children=[p[2], p[5], p[8]], lineno=p.lineno(1))

    def iferr_b(self, p):
        if len(p) == 4:
            p[0] = Node('if error 1', data='You should have THEN statement', lineno=p.lineno(1))
        else:
            p[0] = Node('if error 2', data='You should have ELSE statement', lineno=p.lineno(1))
    ...
    def statement_gr(self, p):
        if len(p) == 2:
            ...
            p[4] = Node('End', data=p[4], lineno=p.lineno(1))
            p[0] = Node('statement group', children=[p[1], p[3], p[4]], lineno=p.lineno(1))

    def dowhile(self, p):
        p[0] = Node('do while', children={'body': p[3], 'condition': p[5]}, lineno=p.lineno(1))
